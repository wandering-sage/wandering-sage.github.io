...
class TextEffect {
	...
	toNextWord() {
		...
		// changes random half of the charcters to gibbrish
		a.every((e, i) => {
			if (i > a.length * 0.5) {
				return false;
			}
		...
		// changes random charcters to gibbrish for 10 times, very fast
		for (var j = 0; j < 10; j++) {
		...
		// changes all the charecter to next words character randomly
		a.forEach((e, i) => {
		...
		// makes the length of cur word equal to next word
		function adjustSpans(all, len) {
			...
			if (t > 0) {
				...
				for (let i = t; i < 0; i++) {
					totalTime += this.delay;
					setTimeout(
						() => this.allSpans()[this.allSpans().length - 1].remove(),
						totalTime
					);
				}
			}
	...
	// creates a span el with anim text class
	spanThis(name) {
...
// smoothly scrolls to the target location n the given duration
...
// finds which section the user is curently at
function findActive() {
...
// updates the highlighted section in navBar
...
function scrollAppear() {
	...
	list.forEach((e) => {
		if (
			e.getBoundingClientRect().top < screenPos &&
			!e.classList.contains("appear-on-scroll")
		) {
...
function scrollMouse() {
	cursor.style.transform = `translateX(${mouseX}px) translateY(${
		mouseY + window.scrollY
	}px) translate(-50%,-50%)`;
	cursorSlow.style.transform = `translateX(${mouseX}px) translateY(${
		mouseY + window.scrollY
	}px) translate(-50%,-50%) scale(${mSize})`;
}
